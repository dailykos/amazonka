...
--
-- * 'ddevDBParameterGroupFamily' - The name of a specific DB parameter group family to return details for. Constraints:     * If provided, must match an existing @DBParameterGroupFamily@ .
--
...
--
-- * 'ddevMaxRecords' - The maximum number of records to include in the response. If more records exist than the specified @MaxRecords@ value, a pagination token (marker) is included in the response so that the remaining results can be retrieved. Default: 100 Constraints: Minimum 20, maximum 100.
describeDBEngineVersions
    :: DescribeDBEngineVersions
describeDBEngineVersions =
  DescribeDBEngineVersions'
...
--
-- * 'ddevrsDBEngineVersions' - Detailed information about one or more DB engine versions.
--
