...
data DescribeStackResourceDrifts = DescribeStackResourceDrifts'
  { _dsrdNextToken :: !(Maybe Text)
  , _dsrdMaxResults :: !(Maybe Nat)
  , _dsrdStackResourceDriftStatusFilters :: !(Maybe (List1 StackResourceDriftStatus))
  , _dsrdStackName :: !Text
              ...
              (\ s h x ->
                 DescribeStackResourceDriftsResponse' <$>
                   (x .@? "NextToken") <*> (pure (fromEnum s)) <*>
                     (x .@? "StackResourceDrifts" .!@ mempty >>=
                        parseXMLList "member"))

instance Hashable DescribeStackResourceDrifts where

instance NFData DescribeStackResourceDrifts where
...
instance ToQuery DescribeStackResourceDrifts where
        toQuery DescribeStackResourceDrifts'{..}
          = mconcat
              ["Action" =:
                 ("DescribeStackResourceDrifts" :: ByteString),
...
data DescribeStackResourceDriftsResponse = DescribeStackResourceDriftsResponse'
  ...
  , _drsStackResourceDrifts :: ![StackResourceDrift]
  } deriving (Eq, Read, Show, Data, Typeable, Generic)
