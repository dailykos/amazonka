...
instance AWSPager ListResolversByFunction where
        page rq rs
          ...
          | otherwise =
            Just $ rq & lrbfNextToken .~ rs ^. lrbfrsNextToken

instance AWSRequest ListResolversByFunction where
        type Rs ListResolversByFunction =
             ListResolversByFunctionResponse
        request = get appSync
        response
          = receiveJSON
...
instance Hashable ListResolversByFunction where

instance NFData ListResolversByFunction where
...
instance ToPath ListResolversByFunction where
        toPath ListResolversByFunction'{..}
          = mconcat
              ["/v1/apis/", toBS _lrbfApiId, "/functions/",
               toBS _lrbfFunctionId, "/resolvers"]

instance ToQuery ListResolversByFunction where
        toQuery ListResolversByFunction'{..}
          = mconcat
              ["nextToken" =: _lrbfNextToken,
...
instance NFData ListResolversByFunctionResponse where
