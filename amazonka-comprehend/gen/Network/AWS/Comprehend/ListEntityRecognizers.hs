...
instance AWSPager ListEntityRecognizers where
        page rq rs
          ...
          | stop (rs ^. lerrsEntityRecognizerPropertiesList) =
            Nothing
          | otherwise =
            Just $ rq & lerNextToken .~ rs ^. lerrsNextToken

instance AWSRequest ListEntityRecognizers where
        type Rs ListEntityRecognizers =
             ListEntityRecognizersResponse
...
instance Hashable ListEntityRecognizers where

instance NFData ListEntityRecognizers where
...
instance NFData ListEntityRecognizersResponse where
