...
instance AWSPager ListKeyPhrasesDetectionJobs where
        page rq rs
          ...
          | stop
              (rs ^. lkpdjrsKeyPhrasesDetectionJobPropertiesList)
            = Nothing
          | otherwise =
            Just $ rq & lkpdjNextToken .~ rs ^. lkpdjrsNextToken
...
instance Hashable ListKeyPhrasesDetectionJobs where

instance NFData ListKeyPhrasesDetectionJobs where
