...
instance AWSPager ListSentimentDetectionJobs where
        page rq rs
          ...
          | stop
              (rs ^. lsdjrsSentimentDetectionJobPropertiesList)
            = Nothing
          | otherwise =
            Just $ rq & lsdjNextToken .~ rs ^. lsdjrsNextToken
...
instance Hashable ListSentimentDetectionJobs where

instance NFData ListSentimentDetectionJobs where
