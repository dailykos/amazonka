...
--
-- * 'bLanguageCode' - The language of the input documents. You can specify English ("en") or Spanish ("es"). All documents must be in the same language.
batchDetectSyntax
...
bTextList = lens _bTextList (\ s a -> s{_bTextList = a}) . _Coerce
...
instance Hashable BatchDetectSyntax where

instance NFData BatchDetectSyntax where
...
instance NFData BatchDetectSyntaxResponse where
