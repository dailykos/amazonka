...
instance AWSPager ListDocumentClassificationJobs
         where
        page rq rs
          | stop (rs ^. ldcjrsNextToken) = Nothing
          | stop
              (rs ^. ldcjrsDocumentClassificationJobPropertiesList)
            = Nothing
          | otherwise =
            Just $ rq & ldcjNextToken .~ rs ^. ldcjrsNextToken

instance AWSRequest ListDocumentClassificationJobs
         where
...
instance NFData ListDocumentClassificationJobs where

instance ToHeaders ListDocumentClassificationJobs
         where
        toHeaders
          = const
              (mconcat
