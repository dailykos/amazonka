...
--
-- * 'cfTags' - The tags to associate with the fleet. A tag is a key-value pair (the value is optional). For example, Environment=Test, or, if you do not specify a value, Environment=.  If you do not specify a value, we set the value to an empty string. For more information, see <http://docs.aws.amazon.com/appstream2/latest/developerguide/tagging-basic.html Tagging Your Resources> in the /Amazon AppStream 2.0 Developer Guide/ .
--
          ...
          = object
              (catMaybes
                 [("DomainJoinInfo" .=) <$> _cfDomainJoinInfo,
                  ...
                  ("Tags" .=) <$> _cfTags, Just ("Name" .= _cfName),
