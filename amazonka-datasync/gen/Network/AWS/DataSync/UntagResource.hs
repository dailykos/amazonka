...
--
-- * 'urResourceARN' - The Amazon Resource Name (ARN) of the resource to remove the tag from.
--
-- * 'urKeys' - The keys in the key-value pair in the tag to remove.
untagResource
    :: Text -- ^ 'urResourceARN'
    -> NonEmpty Text -- ^ 'urKeys'
    -> UntagResource
untagResource pResourceARN_ pKeys_ =
  UntagResource' {_urResourceARN = pResourceARN_, _urKeys = _List1 # pKeys_}
...
urKeys :: Lens' UntagResource (NonEmpty Text)
urKeys = lens _urKeys (\ s a -> s{_urKeys = a}) . _List1
...
instance ToHeaders UntagResource where
        toHeaders
          = const
              (mconcat
                 ["X-Amz-Target" =#
                    ("FmrsService.UntagResource" :: ByteString),
                  "Content-Type" =#
