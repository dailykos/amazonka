...
data TransactWriteItems = TransactWriteItems'
  { _twiReturnConsumedCapacity :: !(Maybe ReturnConsumedCapacity)
  , _twiReturnItemCollectionMetrics :: !(Maybe ReturnItemCollectionMetrics)
...
--
-- * 'twiTransactItems' - An ordered array of up to 10 @TransactWriteItem@ objects, each of which contains a @ConditionCheck@ , @Put@ , @Update@ , or @Delete@ object. These can operate on items in different tables, but the tables must reside in the same AWS account and region, and no two of them can operate on the same item. 
transactWriteItems
    :: NonEmpty TransactWriteItem -- ^ 'twiTransactItems'
    -> TransactWriteItems
...
instance Hashable TransactWriteItems where

instance NFData TransactWriteItems where
...
data TransactWriteItemsResponse = TransactWriteItemsResponse'
  { _twirsItemCollectionMetrics :: !(Maybe (Map Text [ItemCollectionMetrics]))
  , _twirsConsumedCapacity :: !(Maybe [ConsumedCapacity])
...
transactWriteItemsResponse
    :: Int -- ^ 'twirsResponseStatus'
    -> TransactWriteItemsResponse
transactWriteItemsResponse pResponseStatus_ =
  TransactWriteItemsResponse'
    { _twirsItemCollectionMetrics = Nothing
...
instance NFData TransactWriteItemsResponse where
