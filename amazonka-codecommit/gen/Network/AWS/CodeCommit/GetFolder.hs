...
--
-- * 'gfCommitSpecifier' - A fully-qualified reference used to identify a commit that contains the version of the folder's content to return. A fully-qualified reference can be a commit ID, branch name, tag, or reference such as HEAD. If no specifier is provided, the folder content will be returned as it exists in the HEAD commit.
--
-- * 'gfRepositoryName' - The name of the repository.
--
-- * 'gfFolderPath' - The fully-qualified path to the folder whose contents will be returned, including the folder name. For example, /examples is a fully-qualified path to a folder named examples that was created off of the root directory (/) of a repository. 
getFolder
...
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'gfrsSubModules' - The list of submodules that exist in the specified folder, if any.
--
-- * 'gfrsTreeId' - The full SHA-1 pointer of the tree information for the commit that contains the folder.
--
-- * 'gfrsSubFolders' - The list of folders that exist beneath the specified folder, if any.
--
-- * 'gfrsSymbolicLinks' - The list of symbolic links to other files and folders that exist in the specified folder, if any.
--
-- * 'gfrsFiles' - The list of files that exist in the specified folder, if any.
--
-- * 'gfrsResponseStatus' - -- | The response status code.
--
-- * 'gfrsCommitId' - The full commit ID used as a reference for which version of the folder content is returned.
--
