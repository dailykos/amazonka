...
instance AWSPager ListAggregateDiscoveredResources
         where
        page rq rs
          | stop (rs ^. ladrrsNextToken) = Nothing
          | stop (rs ^. ladrrsResourceIdentifiers) = Nothing
          | otherwise =
            Just $ rq & ladrNextToken .~ rs ^. ladrrsNextToken

instance AWSRequest ListAggregateDiscoveredResources
         where
...
instance ToHeaders ListAggregateDiscoveredResources
         where
        toHeaders
          = const
              (mconcat
...
instance ToJSON ListAggregateDiscoveredResources
         where
...
instance ToPath ListAggregateDiscoveredResources
         where
        toPath = const "/"

instance ToQuery ListAggregateDiscoveredResources
         where
